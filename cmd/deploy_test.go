...
`)},
	...
	if f = deploy(t, "--revision=main", "--builder=pack"); f.Build.Builder != "pack" {
	...
	if f = deploy(t, "--revision=main", "--namespace=repo-ns"); f.Namespace != "repo-ns" {
		t.Errorf("expected the namespace from the flag, got %q", f.Namespace)
	}
	cmd := NewDeployCmd(NewTestClient(fn.WithPipelinesProvider(mock.NewPipelinesProvider()), fn.WithRegistry(TestRegistry)))
	cmd.SetArgs([]string{"--remote", "--source=" + url, "--revision=main", "--namespace=flag-ns"})
